...
class Solver:
    def __init__(self,filename,random_parameters,run_num=0):
...
#            # Create objective function
            ...
            m.setObjective((gurobipy.quicksum(t*x[self.n_jobs-1,mode,t] for mode in self.job_modes[self.n_jobs-1]
                                                                        ...
                                                                        for t in range(self.job_early_start[-1],
                                                                                       self.horizon + 1
#                                                                                       self.job_late_start[-1]+1
                                                                                       )
                                                                        ))
                                                    ,GRB.MINIMIZE)
            ...
            for i in range(self.n_jobs-1):
                for j in self.successor[i]:
                    if(i != 0 and j != self.n_jobs - 1):
                        m.addConstr(gurobipy.quicksum(t*x[j,mode,t] for t in range(self.job_early_start[j],
#                                                      self.job_late_start[j]+1
                                                      self.horizon+1
                                                      )
                            ...
                            >=  gurobipy.quicksum((t + self.duration_mode[i][mode])*x[i,mode,t]
                             ...
                             #+  gurobipy.quicksum(1*theta[mode,j,i] for mode in self.job_modes[i])
...
#                                                          self.job_late_start[j]+1
                                                          self.horizon+1
                                                                ...
                                                                for mode in self.job_modes[j])
                            >=  gurobipy.quicksum((t + self.duration_mode[i][mode])*x[i,mode,t]
                                                                                                 for t in range(self.job_early_start[i],
#                                                                                                                self.job_late_start[j]+1
                                                                                                                self.horizon+1
                                                                                                                )
                                                                                                 for mode in self.job_modes[i])
...
gurobipy.quicksum((self.job_mode_resource[i][mode][res]*x[i,mode,tau] for i in range(1,self.n_jobs-1)
                                                                      for mode in self.job_modes[i]
                                                                      for tau in range(self.job_early_start[i],
#                                                                                       max([self.job_late_start[suc] for suc in self.successor[i]])+1
                                                                                       self.horizon+1
                                                                                       )
                                                                      )
            ...
            m.addConstrs(x[i,mode,t]  - y[i,mode,t+self.duration_mode[i][mode]] == 0
                                                                                                for i in range(self.n_jobs)
                                                                                                for mode in self.job_modes[i]
                                                                                                for t in range(self.job_early_start[i],self.job_late_start[i] - self.duration_mode[i][mode])
                                                                                                        
                        )
            
            # First constraint of fixed locations extension
            m.addConstrs(gurobipy.quicksum(x[i,mode,t] + y[i,mode,t+self.duration_mode[i][mode]] for t in range(self.job_early_start[i],self.job_late_start[i] - self.duration_mode[i][mode])) 
                                    <= 1 + theta[mode,j,i] 
                                                           for i in range(self.n_jobs-1)
                                                           for j in self.successor[i]
                                                           for mode in self.job_modes[i]
            ...
            # Naive constraints
            # Fifth constraint
            m.addConstrs(gamma_f[j,l1] + gamma_s[i,l2] <= 1 + beta[j,i,l1,l2] for i in range(self.n_jobs-1)
                                                                              for j in self.successor[i]
                                                                              for l1 in range(self.n_loc)
                                                                              for l2 in range(self.n_loc) )
            # Sixth constraint
            m.addConstrs(beta[j,i,l1,l2] + gurobipy.quicksum(theta[mode,j,i] for mode in self.job_modes[i])
                          <= 1 + delta[l1,l2,j,i]                            for i in range(self.n_jobs - 1)
                                                                             for j in self.successor[i]
                                                                             for l1 in range(self.n_loc)
                                                                             for l2 in range(self.n_loc))
